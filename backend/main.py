...
# --- 1. 프로젝트 모듈 import ---
from database import db # db 객체만 임포트
...
app = FastAPI(
    title="News IT's Issue API (v2.2.4)", # 버전 업데이트
...
# --- 4. 데이터베이스 초기화 ---
# @app.on_event("startup") 에서 db.init_database()가 호출되므로 별도 호출 불필요
...
async def debug_info():
    db_path = db.db_path # db 객체에서 경로를 가져옴
