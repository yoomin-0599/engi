        ...
        try:
            ...
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS articles (
                    ...
                    keywords TEXT,
                    raw_text TEXT,
                    main_category TEXT, -- [추가] 대분류 컬럼
                    sub_category TEXT,  -- [추가] 소분류 컬럼
                    language TEXT,
        ...
        try:
            ...
            if existing:
                cursor.execute("""
                    ...
                    WHERE id = ?
                """, (
                    article['title'], article['summary'], keywords_json, article.get('raw_text', ''),
                    article.get('main_category', '기타'), article.get('sub_category', '기타'),
                    existing['id']
                ))
                result = "updated"
            else:
                cursor.execute("""
                    INSERT INTO articles (title, link, published, source, summary, keywords, raw_text, main_category, sub_category, language)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    article['title'], article['link'], article['published'],
                    article['source'], article['summary'], keywords_json,
                    article.get('raw_text', ''), article.get('main_category', '기타'),
                    article.get('sub_category', '기타'), article.get('language', '')
                ))
                result = "inserted"
